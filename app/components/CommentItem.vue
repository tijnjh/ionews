<script setup lang="ts">
  ...
  const { comment, level = 0 } = defineProps<CommentItemProps>();
...
<template>
  ...
  >
    ...
    <div class="px-4 py-2 w-full">
      <IonLabel>
        <p class="flex items-center cursor-pointer" @click="toggleCollapse()">
          {{ comment.user }}
          ...
          {{ comment.time_ago }}
          <span class="ml-auto">
            <IonIcon :icon="!isCollapsed ? ioniconsChevronUp : ioniconsChevronDown" />
          </span>
        </p>
        <AnimateHeight :height="height" :duration="animateDuration">
          <IonText>
            <div
